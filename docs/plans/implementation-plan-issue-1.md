...
- [ ] 提供されたJSONスキーマに基づく、過不足のない型定義が用意されている。
- [ ] `CiniiApiClient` をインスタンス化し、全7種のエンドポイントに対して検索を実行できる。
- [ ] 検索時に必須となる `appid` パラメータが透過的かつ自動的に付与される。
- [ ] HTTPレスポンスが正しく `CiniiApiResponse` 型のオブジェクトに変換される。
- [ ] 同一の検索リクエストを再度実行した際、キャッシュファイルからデータがロードされる。
- [ ] 通信エラーや不正なフォーマットを受け取った際、適切なカスタム例外がスローされる。
